...
askKeystorePassword().then(result => {
  ...
  exec(
    ...
    `tns build android --release --bundle --env.aot --env.uglify --key-store-path ./tools/smarteval-keystore.jks --key-store-password ${result} --key-store-alias upload --key-store-alias-password ${result} --copy-to ./tools/smart-eval.apk`,
    (err, stdout, stderr) => {
      if (err) {
