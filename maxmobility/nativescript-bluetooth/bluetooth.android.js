  ...
  if (android.os.Build.VERSION.SDK_INT >= 21 /*android.os.Build.VERSION_CODES.LOLLIPOP */) {
    ...
    var MyGattServerCallback = android.bluetooth.BluetoothGattServerCallback.extend({
      onCharacteristicWriteRequest: function(
        ...
        console.log(
          '----- _MyGattServerCallback.onCharacteristicWriteRequest, device: ' + device + ', requestId: ' + requestId
        );
      ...
      onCharacteristicReadRequest: function(device, requestId, offset, characteristic) {
        console.log(
          '----- _MyGattServerCallback.onCharacteristicReadRequest, device: ' + device + ', requestId: ' + requestId
        );
        _onCharacteristicReadRequestCallback &&
          _onCharacteristicReadRequestCallback(device, requestId, offset, characteristic);
        let status = 0;
        if (gattServer !== null && gattServer !== undefined) {
          gattServer.sendResponse(device, requestId, status, offset, new Array([0x01]));
        }
      ...
      onDescriptorWriteRequest: function(device, requestId, descriptor, preparedWrite, responseNeeded, offset, value) {
        console.log(
          '----- _MyGattServerCallback.onDescriptorWriteRequest, device: ' + device + ', requestId: ' + requestId
        );
        _onDescriptorWriteRequestCallback &&
          _onDescriptorWriteRequestCallback(
            device,
            requestId,
            descriptor,
      ...
      onDescriptorReadRequest: function(device, requestId, offset, descriptor) {
        console.log(
          '----- _MyGattServerCallback.onDescriptorReadRequest, device: ' + device + ', requestId: ' + requestId
        );
      ...
      onConnectionStateChange: function(device, status, newState) {
        console.log(
          '----- _MyGattServerCallback.onConnectionStateChange, device: ' +
            device +
            ', status: ' +
            status +
            ', newState: ' +
        ...
        if (!stateObject) {
          ...
          if (
            ...
            manufacturerId = result
              ...
              .getManufacturerSpecificData()
              .keyAt(0);
            manufacturerData = Bluetooth._decodeValue(
              result
                .getScanRecord()
                .getManufacturerSpecificData()
                .valueAt(0)
...
Bluetooth.makeCharacteristic = function(characteristicOptions) {
  ...
  let gprop = new Number(
    characteristicOptions.gattProperty || android.bluetooth.BluetoothGattCharacteristic.PROPERTY_READ
  );
  let gperm = new Number(
...
Bluetooth.startAdvertising = function(advertiseOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (adv === null || !adapter.isMultipleAdvertisementSupported()) {
      ...
      let _d = new android.bluetooth.le.AdvertiseData.Builder().addServiceUuid(pUuid).build();
...
Bluetooth.read = function(arg) {
  return new Promise(function(resolve, reject) {
    try {
      ...
      var bluetoothGattCharacteristic = Bluetooth._findCharacteristicOfType(
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      var bluetoothGattCharacteristic = Bluetooth._findCharacteristicOfType(
        ...
        Bluetooth._stringToUuid(arg.characteristicUUID),
